...
namespace ProjectManagerBackend.Test.Controllers
{
    public class ClientControllerTest
    {
        ...
        public async Task GetAllDepartments_ReturnsOkResultAndDepartmentsList()
        {
            ...
            var controller = new GenericController<Client, ClientDTO, ClientDTO>(_repository, _mapping, _validationService);
        ...
        public async Task CreateClient_ReturnOkResultAndClient()
        {
            ...
            // Act
            var result = await controller.Create(_singleClientDTO); // Call Create method with _singleClientDTO as parameter
            ...
            var returnedModel = Assert.IsType<OkObjectResult>(result.Result); // Verify if the result is of type OkObjectResult
            Assert.NotNull(returnedModel); // Check if the returned model is not null
